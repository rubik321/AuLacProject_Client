...
public class LevelPlayAds : MonoBehaviour
{
   ...
   // public TextMeshProUGUI txtMess;
    ...
    public void OnShowReward(Action onDone = null)
    {
        if(UserDataManager.Instance.IsPurchaseRemoveAds())
        {
            onDone?.Invoke();
            return;
        }
        ...
        {
            OnLoadReward();
            HUDCanvas.Instance.ShowNotification("Rewarded Video Ad is not ready !!!", "Message", null);
    ...
    {
        Debug.LogError($"[LevelPlaySample] Received SdkInitializationCompletedEvent with Config: {config}");
        EnableAds();
        isAdsEnabled = true;
    }
    ...
    {
        Debug.LogError($"[LevelPlaySample] Received SdkInitializationFailedEvent with Error: {error}");
    }
