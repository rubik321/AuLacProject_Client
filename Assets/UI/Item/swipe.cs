            ...
            if (scroll_pos < pos[i] + (distance / 2) && scroll_pos > pos[i] - (distance / 2))
            {
                ...
                for (int j = 0; j < pos.Length; j++)
                {
                    if (j != i)
                    {
                        ...
                        imageContent.transform.GetChild(j).localScale = Vector2.Lerp(imageContent.transform.GetChild(j).localScale, new Vector2(0.95f, 0.95f), 0.1f);
                        transform.GetChild(j).localScale = Vector2.Lerp(transform.GetChild(j).localScale, new Vector2(0.95f, 0.95f), 0.1f);
                    }
                }
            }
        }
